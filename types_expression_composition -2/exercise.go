//NOTE
//Here We going to Count Boolean Expression
...
func Play() {
	...
	stillInOffice := true //NO 1

	// play for 10 years, or until kicked out
	for stillInOffice && year <= 10 { //NO 2
		...
		if countStarvedEmployees() >= 45 { //NO 3
			stillInOffice = false
		}
...
func printSummary() {
	...
	if amountStolenByHackers > 0 { //NO 5
...
func sellComputers() {
	...
	for computersToSell > computers { //NO 7
...
func payEmployees() {
	...
	for cashPaidToEmployees > cash { //NO 8
...
func countStarvedEmployees() int {
	...
	if employeesPaid >= employees { //NO 17
		starved = 0
...
func printFinalScore() {
	...
	if starved >= (45*employees)/100 { //NO 18
...
Your final rating: TERRIBLE.`, OGH, starved)
	...
	if 20*employees < computerScore { //NO 19
		computerScore = 20 * employees
	}

	if computerScore < 600 { //NO 20
...
Your final rating: ADEQUATE`, OGH, computers)
	} else if computerScore < 800 { //NO 21
...
func clearScreen() {
	if strings.Contains(runtime.GOOS, "windows") { //NO 25
		// windows
...
//Therefore there are 26 boolean expressions in this project
